<template>
  <v-expansion-panels v-model="panelOpenRef" flat multiple accordion>
    <v-expansion-panel
      ...
      v-for="({ id, title, sectionData }, index) in devisParticulierComputed.infoDevisSections"
      :key="index"
    >
      ...
      <v-expansion-panel-content>
        ...
        <ListeResSecondConsultDevis v-if="showListeDescriptionConsultDevis(id)" :devis="devisRef" />
...
import ListeResSecondConsultDevis from './liste/ListeResSecondConsultDevis.vue';
...
const { getResumeDevis, getSouscripteurPhysique, getRiskForParticulier } = getListeConsultDevis();
...
const devisParticulierComputed = computed(() => {
  const devisParticulierModel = {
    infoDevisHeader: {
    ...
    infoDevisSections: [
      ...
      getSouscripteurPhysique(devisRef.value),
      getRiskForParticulier(devisRef.value)
    ]
  };
  return devisParticulierModel;
});
...
const showListeDescriptionConsultDevis = computed(() => {
  return (id: string): boolean =>
    id === DESCRIPTION_DEVIS && devisRef.value.residencesSecondaires?.length !== 0;
});
