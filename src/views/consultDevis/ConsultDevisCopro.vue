<template>
  <v-expansion-panels v-model="panelOpenRef" flat multiple accordion>
    <v-expansion-panel
      ...
      v-for="({ id, title, sectionData }, index) in devisCoproComputed.infoDevisSections"
      :key="index"
    >
...
const { getResumeDevis, getSouscripteurMoral, getBeneficiaireMoralForCopro, getRiskForCopro } =
...
const devisCoproComputed = computed(() => {
  const devisCoproModel = {
    infoDevisHeader: {
    ...
    infoDevisSections: [
      ...
      getBeneficiaireMoralForCopro(devisRef.value),
      getRiskForCopro(devisRef.value)
    ]
  };

  return devisCoproModel;
});
