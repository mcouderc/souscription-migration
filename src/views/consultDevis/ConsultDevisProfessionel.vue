<template>
  <v-expansion-panels v-model="panelOpenRef" flat multiple accordion>
    <v-expansion-panel
      ...
      v-for="({ id, title, sectionData }, index) in devisProfessionelComputed.infoDevisSections"
      :key="index"
    >
      ...
      <v-expansion-panel-content>
        ...
        <div v-if="showListeDescriptionConsultDevis(id)">
          <ListeScisConsultDevis v-if="devisRef.scis?.length !== 0" :devis="devisRef" />
          <ListeFilialesConsultDevis v-if="devisRef.filiales?.length !== 0" :devis="devisRef" />
          <ListeEtablissementsSecondaires
            v-if="devisRef.etablissementsSecondaires?.length !== 0"
            :devis="devisRef"
          />
        </div>
        <ListeErrorsConsultDevis
          ...
          :conditionSpec="devisRef.conditionsSpecifiques"
        />
      </v-expansion-panel-content>
    </v-expansion-panel>
  </v-expansion-panels>
...
import ListeScisConsultDevis from './liste/ListeScisConsultDevis.vue';
import ListeEtablissementsSecondaires from './liste/ListeEtablissementsSecondaires.vue';
...
const { getResumeDevis, getSouscripteurMoral, getRiskForProfessionel } = getListeConsultDevis();
...
const devisProfessionelComputed = computed(() => {
  const devisProfessionelModel = {
    infoDevisHeader: {
    ...
    infoDevisSections: [
      ...
      getRiskForProfessionel(devisRef.value)
    ]
  };

  return devisProfessionelModel;
});
...
const showListeDescriptionConsultDevis = computed(() => {
  return (id: string): boolean => id === DESCRIPTION_DEVIS;
});
